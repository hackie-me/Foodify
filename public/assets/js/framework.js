    ...
    var CosticFramework = (function () {
        ...
        function deletableItem() {
            $('body').on('click', '.ms-delete-trigger', function () {
                const $deletable = $(this).closest('.ms-deletable');
                // Check if element is a block or a task
                if ($deletable.hasClass('ms-qa-card')) {
                    const data = {
                        id: $(this).closest('.ms-qa-card').find('.ms-card-block-id').val(),
                        type: "block"
                    }
                    $.ajax({
                        url: '/api/app/todo/delete',
                        type: 'DELETE',
                        data: data,
                ...
                if ($deletable.hasClass('ms-qa-task')) {
                    // alert('Task Deleted');

                    const data = {
                        id: $(this).closest('.ms-qa-task').find('.ms-card-task-id').val(),
                        type: "task"
                    }
                    $.ajax({
                        url: '/api/app/todo/delete',
        ...
        function archiveTask() {
            $('body').on('click', '.ms-archive-trigger', function () {
                const $deletable = $(this).closest('.ms-deletable');
                // if this block is already archived, then unarchive it
                let status = true;
                if ($deletable.hasClass('archived')) {
                    status =  false
                }
                const data = {
                    id: $(this).closest('.ms-qa-card').find('.ms-card-block-id').val(),
        ...
        // Function to show only archived tasks
        ...
        function showAllTodo(){
            $('.show-all-todo').on('click', function () {
                $('.archived').show();
                $('.not-archived').show();
            });
        }
        ...
        function addTaskBlockApp() {
            $(".ms-add-task-block-2").on('submit', function (e) {
                e.preventDefault();
                // Call Api to add task block to database
                $.post("/api/app/todo/add/block", {
                    name: $("#task-block-2").val(),
                    user: $("#uid").val()
                }, function (data, status) {
                    // if response code is 201 then add task block to UI
                    ...
                    if (status == "success") {
                        $(".ms-todo-list").prepend(
                            ...
                            '<h6 class="ms-card-title">' + $("#task-block-2").val() + '<input type="hidden" class="ms-card-block-id" value="'+response.id+'"></input>' +'</h6> <button data-toggle="tooltip" data-placement="left" title="Add a Task to this block" class="ms-add-task-to-block ms-btn-icon float-right">' +
                            '<i class="material-icons">add</i> </button> </div> <div class="ms-card-body"> <ul class="ms-list ms-task-block"> </ul></div>' +
                            '<div class="ms-card-footer clearfix"><a href="#" class="text-disabled mr-2"> <i class="flaticon-archive"> </i> Archive </a><a href="#" class="text-disabled ms-delete-trigger float-right">' +
        ...
        function addTask() {
            $('.ms-todo-list').on('click', '.ms-add-task-to-block', function () {
                ...
                taskBlock.append(
                    ...
                    '</label>' +
                    '<input type="hidden" class="ms-card-task-id" value=""></input>' +
        ...
        function confirmTask() {
            $(".ms-todo-list").on('submit', '.ms-confirm-task-form', function (e) {
                e.preventDefault();
                const $this = $(this); // Store $(this) in a variable for later use
                // Call Api to add task to database
                $.post("/api/app/todo/add/task", {
                    name: $this.find('.ms-task-input').val(),
                    user: $("#uid").val(),
                    block_id: $this.closest('.ms-card').find('.ms-card-block-id').val()
                }, function (data, status) {
                    const response = data.data
                    if (status == "success") {
                        const confirmBtn = $this.find('i');
                        const taskInput = $this.find('.ms-task-input');
                        confirmBtn.removeClass('material-icons fs-14 ms-confirm-trigger');
                        confirmBtn.addClass('');
                        $this.closest('.ms-list-item').find('.ms-card-task-id').val(response.id);
                        confirmBtn.text('');
        ...
        function taskComplete() {
            $(".ms-todo-list").on('click', '.ms-todo-complete', function () {
                // Call Api to update task status to database
                const data = {
                    id: $(this).closest('.ms-list-item').find('.ms-card-task-id').val(),
                    status: $(this).find('input').is(':checked')
                }
                const item = $(this).closest('.ms-list-item');
                $.post("/api/app/todo/completed", data, function (data, status) {
                    if (status === "success") {
                        item.toggleClass('task-complete');
                    } else {
                        console.log(data.message);
                    }
                });
